...
class TpgAppContext(AppContext):
    ...
    def init_context(self):
        ...
        if 'asset_name' in self._ext_conf:
            self.asset_name = self._ext_conf['asset_name']    
    ...
    def run_context(self):
        ...
        for equ, tag in self._vtag_subscribe.items():    
    ...
    def build_mqtt_msg_charlie(self):        
        ...
        while len(self._vtag_data)>0:
            ...
            payload['metrics'][tag['name']] = int(tag['value'])
            payload['metrics'][tag['name'] + "_timestamp"] = tag['timestamp']
        
        return payload 

    def tpg_callback(self, source_name, tag_name, Tag):
        self.log_debug('%s (%s) - %s: Tag %s', source_name, str(Tag.at()), tag_name, Tag.value().as_int())
        self._vtag_tags += 1
        timestamp=int(time.time()*1000)
        self._vtag_data.append({'name': tag_name, 'value': Tag.value().as_int(), 'timestamp': timestamp})
        return
